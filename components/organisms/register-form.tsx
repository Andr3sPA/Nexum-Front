...
import { Eye, EyeOff, Mail, Lock, User, Calendar, CreditCard, Users, Building, Phone } from "lucide-react"
...
export interface RegisterFormProps {
  ...
  onEmployerSubmit?: (data: EmployerFormData, editCode?: string) => Promise<void>
  error?: string | null
  isLoading?: boolean
...
export function RegisterForm({
  ...
  const [employerFormData, setEmployerFormData] = useState<EmployerFormData>({
    contactName: initialEmployerData.contactName || "",
    email: initialEmployerData.email || "",
    phone: initialEmployerData.phone || "",
    businessName: initialEmployerData.businessName || "",
    nit: initialEmployerData.nit || "",
    password: "",
    confirmPassword: ""
  })
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isEmployer) {
      ...
      if (employerFormData.password !== employerFormData.confirmPassword) {
      ...
      try {
        if (onEmployerSubmit) {
          await onEmployerSubmit(employerFormData, initialEmployerData?.editCode)
        }
        logger.info("✅ RegisterForm.handleEmployerSubmit completed successfully")
      } catch (error) {
        logger.error("❌ RegisterForm.handleEmployerSubmit error:", error)
      }
    } else {
      // Validate passwords match
  ...
  const handleEmployerInputChange = (field: keyof EmployerFormData, value: string) => {
    setEmployerFormData(prev => ({ ...prev, [field]: value }))
  }
    ...
    <div className="w-full space-y-4">
      ...
      <div className="text-center space-y-2">
        ...
        <h1 className="text-2xl font-bold text-gray-900">
          {isEmployer ? "Registro de Empleador" : "Crear Cuenta"}
        </h1>
        <p className="text-gray-600 text-sm">
          {isEmployer
            ? "Completa la información para crear tu cuenta como empleador"
          ...
          <>
            ...
            <div className="space-y-3">
              <div className="border-b border-gray-200 pb-2">
                <h2 className="text-base font-semibold text-gray-900">Información de Contacto</h2>
              </div>

              <div className="space-y-3">
                {/* Contact Name and Email */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <FormField id="contactName" label="Nombre de Contacto" required>
                    <div className="relative">
                      <User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                      <Input
                        id="contactName"
                        value={employerFormData.contactName}
                        onChange={(e) => handleEmployerInputChange("contactName", e.target.value)}
                  ...
                  <FormField id="email" label="Correo Electrónico" required>
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleEmployerInputChange("email", e.target.value)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <FormField id="phone" label="Teléfono">
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleEmployerInputChange("phone", e.target.value)}
                  ...
                  <FormField id="businessName" label="Nombre de la Empresa" required>
                    <div className="relative">
                      <Building className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                      <Input
                        id="businessName"
                        value={employerFormData.businessName}
                        onChange={(e) => handleEmployerInputChange("businessName", e.target.value)}
                ...
                {/* NIT */}
                <FormField id="nit" label="NIT">
                  <Input
                    id="nit"
                    value={employerFormData.nit}
                    onChange={(e) => handleEmployerInputChange("nit", e.target.value)}
                  ...
                  <FormField id="password" label="Contraseña" required>
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleEmployerInputChange("password", e.target.value)}
                        placeholder="••••••••"
                  ...
                  <FormField id="confirmPassword" label="Confirmar" required>
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleEmployerInputChange("confirmPassword", e.target.value)}
                        placeholder="••••••••"
                        className="pl-10 pr-10 h-10"
                        required
                        disabled={isLoading}
        ...
        >
          {isLoading ? (
            <div className="flex items-center space-x-2">
              ...
              <span>{isEmployer ? "Registrando empleador..." : "Creando cuenta..."}</span>
            </div>
          ) : (
            isEmployer ? "Registrar Empleador" : "Crear Cuenta"
          )}
        </Button>
      </form>
    </div>
